# InverseModelMulti.py
# Script to fit various cochlear implant measures using the model by Goldwyn et al. (2010) and Perkel et al. (2025)
...
# Major reworking Dec 2025 by DJP to take in multiple types of clinical or research data for fitting.
# These include MP and TP thresholds, most comfortble level (M-level), ecap parameters and perhaps others
...
# fit_using   # The fit_measures variable says how much relative weighting should be placed on each component.
...
def objectivefunc_lmfit_all(par, sigvals, sim_params, f_par, e_field, thr_goals):
    ...
    weights = np.zeros(len(sim_params['run_info']['fit_measures']))
    weights[0] = sim_params['run_info']['fit_measures']['mp']
    weights[1] = sim_params['run_info']['fit_measures']['tp']
    weights[2] = sim_params['run_info']['fit_measures']['mp_mcl']
    weights[3] = sim_params['run_info']['fit_measures']['ecap']

    if weights[0] > 0.0:
        sim_params['channel']['sigma'] = 0.0   # 0 if looking at M levels
        sim_params['channel']['behavior'] = 0
        thresh_mp = gt.get_thresholds(e_field, f_par, sim_params)
        mp_err = np.nanmean(np.abs(np.subtract(thresh_mp[0], thr_goals['thrmp_db'])))
        mp_diff = np.subtract(thresh_mp[0], thr_goals['thrmp_db'])
    else:
        mp_diff = np.zeros(NELEC)
        mp_err = 0.0
    ...
    if weights[1] > 0.0:
        ...
        tp_diff = np.zeros(NELEC)
        tp_err = 0.0

    if weights[2] > 0.0:
        # mp_mcl stuff here
        sim_params['channel']['sigma'] = 0.0  # 0 if looking at M levels
        sim_params['channel']['behavior'] = 1
        mp_mcl_db = gt.get_thresholds(e_field, f_par, sim_params)
        mp_mcl_diff = np.subtract(mp_mcl_db[0], thr_goals['mp_mcl_db'])
        mcl_err = np.nanmean(np.abs(np.subtract(mp_mcl_db[0], thr_goals['mp_mcl_db'])))
    else:
        mp_mcl_diff = np.zeros(NELEC)
        mcl_err = 0.0
    ...
    if weights[3] > 0.0:
        ...
        ecap = c_ecap.calc_ecap(act_vals, f_par, simParams, ec_thresh, thr_sim_db[:, i], nstim)
        ecap_diff = np.subtract(ecap, thr_goals['ecap'])
        ecap_err = np.nanmean(np.abs(np.subtract(ecap, thr_goals['ecap'])))
    else:
        ecap_diff = np.zeros(NELEC)
        ecap_err = 0.0


    # Calculate errors
    mean_error = np.sum(np.multiply(weights, [mp_err, tp_err, mcl_err, ecap_err]))

    tempzero = np.zeros(1)
    if tp_extend:
        retval = np.concatenate((mp_diff*weights[0], tempzero, tp_diff*weights[1], tempzero,
                                 mp_mcl_diff*weights[2], ecap_diff*weights[3]))
    else:
        retval = np.concatenate((mp_diff[1:-1]*weights[0], tp_diff*weights[1],
                                 mp_mcl_diff*weights[2], ecap_diff*weights[3]))
...
def inverse_model_multi(mode):  # Start this script
    ...
    if not os.path.isdir(INV_OUT_PRFIX):  # if the inverse output directory doesn't exist, make it
    ...
    # TODO: double check that it's the correct 2D data with identical parameters
    pr = cProfile.Profile()
    ...
    for scen in range(0, len(scenarios)):
        ...
        #  TODO this needs updating depending on which data will be used (fit_using variable)
        if use_fwd_model:
            [survVals, electrodes['rpos'], espace] = s_scen.set_scenario(scenario, NELEC)
        else:
            subject = scenario
            retval = subject_data.subj_thr_data(subject)  # TODO should read subject data from a file, not code
        ...
        #  TODO Need to generalize loading 2D data according to 'fit_using'
        if fit_measures['mp'] > 0.0:
        ...
        if fit_measures['mp_mcl'] > 0.0:
            # Load mp_mcl 2D data
            datafile = FWDOUTPUTDIR + "MP_mcl_2D_" + STD_TEXT + es_text + ".csv"
            file = open(datafile)
            numlines = len(file.readlines())
            file.close()
            ...
            with open(datafile, newline='') as csvfile:
                ...
                ncol = len(next(datareader))
                csvfile.seek(0)
                mp_mcl_thr = np.empty([numlines, ncol])
                for i, row in enumerate(datareader):
                    # Do the parsing
                    mp_mcl_thr[i, :] = row

        if fit_measures['ecap'] > 0.0:
            # Load ecap 2D data
            datafile = FWDOUTPUTDIR + "ecap_2D_" + STD_TEXT + es_text + ".csv"
            file = open(datafile)
            numlines = len(file.readlines())
            file.close()
        ...
        if use_fwd_model:
            if fit_measures['mp'] > 0.0:  # get mp and tp data
                csv_file = FWDOUTPUTDIR + 'FwdModelOutput_thresh_' + scenario + '.csv'
                [thr_data, ct_data] = lcsv.load_fwd_csv_data(csv_file, 'mp_tp')
                subject = []
            ...
            if fit_measures['mp_mcl'] > 0.0:
                csv_file = FWDOUTPUTDIR + 'FwdModelOutput_mp_mcl_' + scenario + '.csv'
                [thr_data_mcl, ct_data] = lcsv.load_fwd_csv_data(csv_file, 'mp_mcl')
                thr_data['mp_mcl_db'] = np.array(thr_data_mcl['mp_mcl_db'])

            if fit_measures['ecap'] > 0.0:
                csv_file = FWDOUTPUTDIR + 'FwdModelOutput_mp_mcp_' + scenario + '.csv'
                [thr_data_ecap, ct_data] = lcsv.load_fwd_csv_data(csv_file, 'ecap')
        ...
        # commented out because this assignment needs to be done for each variable type
        # thresh_summary_full[scen, 0, 0, :] = thr_data['thrmp_db']
        # thresh_summary_full[scen, 0, 1, :] = thr_data['thrtp_db']
        ...
        #  TODO Need to update this and restructure. Rather than 'combined' or 'rpos' or 'survival', we should
        #  identify all constraints and fit ENI parameters using all constraints
        if fit_mode == 'combined':  # Optimize survival and rpos to match MP and TP thresholds
            ...
            for i in range(1, NELEC - 1):  # Fit params for each electrode (except ends, where there is no tripol value)
                mptarg = thresh_summary_full[scen, 0, 0, :][i]
                tptarg = thresh_summary_full[scen, 0, 1, :][i]
                ...
                ax5.axes.xaxis.set_label_text('Rpos (mm)')
                ax5.axes.yaxis.set_label_text('Survival fraction')
                ...
                if hasattr(rp_guess, '__len__'):
                    fitrposvals[i] = rp_guess[0]
                else:
                    fitrposvals[i] = rp_guess
                if hasattr(sv_guess, '__len__'):
                    fitsurvvals[i] = sv_guess[0]
                else:
            ...
            pos_slack = 0.05  # amount by which we allow rpos to move
            if use_fwd_model:
                rposvals = electrodes['rpos']
            ...
            if not tp_extend:
                ...
                if use_minimizer == True:
                    for i, val in enumerate(initvec):  # place values in to the par object
                        if i < NELEC - 2:
                            lb = initvec[i] - pos_slack  #0.01# # lower and upper bounds for position #max of lb coordinates max(),-1<--radius of cylinder
                            ub = initvec[i] + pos_slack
                            if lb <= -1 * radius + .05:
                                lb = -1 * radius + .05
                        #  if ub < 0:
                        #ub = .95
                    ...
                    for i, val in enumerate(initvec):
                        if i < 16:
                            lb = rposvals[i] - pos_slack  #0.01 # lower and upper bounds
                            ub = rposvals[i] + pos_slack
                            if lb <= -1 * radius:
                                lb = -1 * radius + .05
                            if ub <= -1 * radius + .95:
                                ub = -1 * radius + .95
                ...
                if use_minimizer == True:
                    for i, val in enumerate(initvec):  # place values in to the par object
                        if i < NELEC:
                            lb = initvec[i] - pos_slack  #CHANGE BACK 0.01# lower and upper bounds for position
                            ub = initvec[i] + pos_slack
                            if lb <= -1 * radius:
                                lb = -1 * radius + .05
                            if ub <= -1 * radius + .95:
                    ...
                    for i, val in enumerate(initvec):
                        if i < 16:
                            lb = rposvals[i] - pos_slack  #CHANGE BACK # lower and upper bounds
                            ub = rposvals[i] + pos_slack
                            if lb <= -1 * radius:
                                lb = -1 * radius + .05
                            if ub <= -1 * radius + .95:
...
if __name__ == '__main__':
    inverse_model_multi(mode='main')  # main or 'survey'
