#  Figs_7_8.py
#  Figs_7_8.py
#  This script plots the subject fitting results for Figs 7 and 8
...
def plot_inverse_results(use_fwd_model, txt_string, unsupervised):
    ...
    axs[0, 0].set_ylim(65, 100)
    axs[0, 0].set_xticklabels([])
    axs[0, 0].set_yticks([70, 80, 90])
    ...
    if use_fwd_model:
        ...
        if np.any(ct_vals):
            axs[1, 0].plot(xvals-0.1, 1 - ct_vals-0.1, marker='o', color='black', label='CT estimate')
...
# Now do second subject
    ...
    axs[0, 1].set_ylim(65, 100)
    axs[0, 1].set_xticklabels([])
    axs[0, 1].set_yticklabels([])
    axs[0, 1].set_yticks([70, 80, 90])
    ...
    if use_fwd_model:
        ...
        if np.any(ct_vals):
            axs[1, 1].plot(xvals-0.1, 1 - ct_vals-0.1, marker='o', color='black', label='CT estimate')
