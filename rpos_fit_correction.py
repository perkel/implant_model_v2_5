...
def fig9_summary():
    ...
    summary_error = np.zeros((nscen, 7))  # for CT and fit dist, slope and intercept, correction
    for idx, scen in enumerate(scenarios[n_artscen:]):  # Panel E
        ...
        if espace == 0.85 or espace == 1.1:
            ...
            # summary_error[idx, 4] = (fit_dist - intercept)/slope
            minx = np.min(ct_dist)
            maxx = np.max(ct_dist)
            if idx == case:
                axs1[2, 0].plot((minx, maxx), (minx*slope + intercept, maxx*slope + intercept), '-', c=fig9_colors[idx, :])  # plot line

    mean_slope = np.mean(summary_error[0, 2])
    mean_intercept = np.mean(summary_error[0, 3])
    ...
    for idx, scen in enumerate(scenarios[n_artscen:]):  # Panel F
        ...
        max_dist = 1.95
        fit_dist = 1 - rpos_fit_vals_0[idx]
        corr_fit_dist = (fit_dist - mean_intercept)/mean_slope
        corr_fit_dist[np.argwhere(corr_fit_dist < min_dist)] = min_dist
        corr_fit_dist[np.argwhere(corr_fit_dist > max_dist)] = max_dist
        # retval = subject_data.subj_thr_data(scen)
        # espace = retval[3]

        if idx == case:
            axs1[2, 1].plot(ct_dist, corr_fit_dist, '.', color=fig9_colors[idx, :])
        axs1[2, 1].set_xlabel('Measured electrode distance (mm)')
        axs1[2, 1].set_ylabel('Corrected fit electrode distance (mm)')
        axs1[2, 1].spines['top'].set_visible(False)
        axs1[2, 1].spines['right'].set_visible(False)
        axs1[2, 1].set_xlim(0, 1.5)

        axs1[2, 1].set_ylim(0.0, 2.0)
        [slope, intercept] = np.polyfit(ct_dist, corr_fit_dist, 1)
    ...
    np.savetxt(INVOUTPUTDIR + 'error_correct.csv', (summary_error[:-3]), delimiter=',')
